...
from datongzi_rules import ConfigFactory, Rank, VariantValidator
...
def test_variant_validator_warnings():
    ...
    config = ConfigFactory.create_custom(
        num_decks=1, num_players=4, cards_dealt_aside=30  # Too many aside
    )
...
def test_config_excluded_ranks():
    ...
    config = ConfigFactory.create_custom(
        num_decks=3, num_players=3, excluded_ranks=excluded
    )
